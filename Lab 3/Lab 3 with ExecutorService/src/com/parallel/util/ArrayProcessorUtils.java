...
public class ArrayProcessorUtils {
    ...
    public int sumElements(int [] array) {
        ...
        for(int i = 0; i < array.length; i++) {
            int finalI = i;
            executorService.execute(() -> processElementByFunction(array[finalI],atomicSum,sum));
        }
    ...
    public int countElementsByPredicate(int [] array, Predicate<Integer> predicate) {
        ...
        for(int i = 0; i < array.length; i++) {
            ...
            executorService.execute(() -> {
                if(predicate.test(array[finalI]))
                processElementByFunction(array[finalI],atomicCounter,iterateCounter);
            });
        }
    ...
    public int calculateCheckSum(int [] array) {
        ...
        for(int i = 0; i < array.length; i++) {
            int finalI = i;
            executorService.execute(() -> processElementByFunction(array[finalI],atomicCheckSum,xor));
        }
    ...
    public int [][] findMinMaxElements (int [] array) {
        ...
        for(int i = 0; i < array.length; i++) {
            int finalI = i;
            executorService.execute(() -> updateMinMax(array,finalI,minIndex,maxIndex));
        }
